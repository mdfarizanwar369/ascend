...
export async function generateRiskAlerts() {
  ...
  await query(`
    ...
    where ra.user_id = u.id
      and ra.status = 'open'
      and ra.type = 'inactive_7_days'
      and (
        exists (select 1 from food_logs fl where fl.user_id = u.id and fl.logged_at > now() - interval '7 days')
        or exists (select 1 from weight_logs wl where wl.user_id = u.id and wl.logged_at > now() - interval '7 days')
        or exists (select 1 from water_logs wat where wat.user_id = u.id and wat.logged_at > now() - interval '7 days')
        or exists (select 1 from habit_logs hl where hl.user_id = u.id and hl.logged_at > now() - interval '7 days')
        or exists (select 1 from analytics_events ae where ae.user_id = u.id and ae.created_at > now() - interval '7 days')
      )
  `);
  ...
  await query(`
    ...
    set status = 'resolved', resolved_at = coalesce(resolved_at, now())
    where ra.status = 'open'
      and ra.type = 'no_food_logs'
      and exists (select 1 from food_logs fl where fl.user_id = ra.user_id and fl.logged_at > now() - interval '3 days')
  `);
  ...
  await query(`
    ...
    where ra.user_id = cs.user_id
      and ra.status = 'open'
      and ra.type = 'low_compliance'
      and cs.calculated_for_date = current_date
      and cs.score >= 50
  `);

  await query(`
    insert into risk_alerts (user_id, trainer_id, gym_id, type, severity, message)
    select distinct u.id, u.assigned_trainer_id, u.gym_id, 'inactive_7_days', 'high',
  ...
  await query(`
    insert into risk_alerts (user_id, trainer_id, gym_id, type, severity, message)
    select distinct u.id, u.assigned_trainer_id, u.gym_id, 'low_compliance', 'high',
  ...
  await query(`
    insert into risk_alerts (user_id, trainer_id, gym_id, type, severity, message)
    select distinct u.id, u.assigned_trainer_id, u.gym_id, 'no_food_logs', 'medium',
  ...
  await query(`
    insert into risk_alerts (user_id, trainer_id, gym_id, type, severity, message)
    select distinct u.id, u.assigned_trainer_id, u.gym_id, 'weight_trend_off_goal', 'medium',
