...
export async function createCoachWorkoutPlan(input: WorkoutPlannerInput): Promise<CoachWorkoutPlan> {
  ...
  try {
    const reply = await createTextReply(
      "You are Coach Zoe inside Ascend, a premium fitness accountability app. Generate one safe workout for today only. Use the provided profile, recent activity, recent workouts, workout memory summary, Health Connect, athlete, food consistency, and conversation context if available. The workout memory summary is important: avoid nearly identical sessions, avoid repeating the same muscle group on consecutive days when the context supports that, and if the latest workout was completed today prefer recovery, mobility, or easy cardio instead of another full session unless the request strongly demands otherwise. Do not build a program. Do not prescribe maximal lifts. Do not give medical advice. Return strict JSON only with keys: title, intro, estimatedDurationMinutes, focus, intensity, warmup, exercises, cooldown, coachTip, disclaimer. exercises must be an array of objects with name, sets, reps, duration, rest, note. Keep it concise, practical, beginner-friendly, and coach-like.",
