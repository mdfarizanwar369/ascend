  ...
  return (
    <section className="mt-4 rounded-lg border border-purple-400/40 bg-purple-400/10 p-4">
      ...
      <form onSubmit={saveTarget} className="rounded-lg bg-ink">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2 rounded-lg bg-surface p-1">{(["daily", "weekly"] as const).map((value) => <button key={value} type="button" onClick={() => setCadence(value)} className={`h-11 rounded-md text-xs font-semibold capitalize ${cadence === value ? "bg-purple-500 !text-white" : "bg-ink !text-zinc-100"}`}>{value}</button>)}</div>
