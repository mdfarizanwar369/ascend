        ...
        <section className="mt-4 space-y-3">
          {items.map((item) => (
            <article key={item.title} className="rounded-lg border border-line bg-surface p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-zinc-400">{iosFree && item.title === "Move" ? "Workouts, movement logs and recorded activity count. A sensible recovery day does not erase your progress." : item.detail}</p>
            </article>
          ))}
        </section>

        {!iosFree && <section className="mt-4 rounded-lg border border-line bg-surface p-4">
