      ...
      <div className="mt-4 space-y-2">
        {sessions.slice(0, 3).map((session) => (
          <article key={session.id} className="rounded-xl border border-white/5 bg-ink p-3">
            ...
            {expanded === session.id ? <div className="mt-3 border-t border-line pt-3"><p className="text-sm leading-6 text-zinc-300">{session.clientRecap}</p><div className="mt-3 space-y-2">{session.exercises.map((exercise, index) => <div key={`${exercise.name}-${index}`} className="flex items-start gap-2 rounded-xl bg-surface p-3"><CheckCircle2 className="mt-0.5 shrink-0 text-lime" size={18} /><div><p className="font-medium">{exercise.name}</p><p className="text-sm text-zinc-500">{[exercise.sets ? `${exercise.sets} sets` : null, exercise.reps ? `${exercise.reps} reps` : null, exercise.load !== null ? `${exercise.load}${exercise.loadUnit ?? ""}` : null].filter(Boolean).join(" / ")}</p></div></div>)}</div><div className="mt-3 rounded-xl border border-calm/20 bg-calm/10 p-3"><p className="text-xs font-bold uppercase tracking-[0.14em] text-calm">Next focus</p><p className="mt-1 text-sm leading-6 text-zinc-200">{session.betweenSessionFocus}</p></div></div> : null}
          </article>
        ))}
      </div>
    </section>
