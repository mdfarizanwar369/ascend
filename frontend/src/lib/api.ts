...
export async function api<T>(path: string, options: RequestInit = {}, token?: string): Promise<T> {
  ...
  try {
    ...
    throw new Error(englishMessage("errors.apiReachAscend"));
  }
  ...
  if (!response.ok) {
    ...
    const message =
      typeof errorBody?.error === "string"
        ...
        : englishMessage("errors.apiRequestFailed", { status: response.status });
...
export async function apiBlob(path: string, options: RequestInit = {}, token?: string) {
  ...
  if (!response.ok) {
    const errorBody = await response.json().catch(() => null);
    const message = typeof errorBody?.error === "string" ? errorBody.error : englishMessage("errors.apiRequestFailed", { status: response.status });
