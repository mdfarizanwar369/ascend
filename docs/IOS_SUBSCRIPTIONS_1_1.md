...
| Product | Identifier | Target Malaysia monthly price |
| --- | --- | --- |
| Ascend Premium | `fit.getascend.app.premium.monthly` | MYR 19.99 |
| Ascend Trainer Pro | `fit.getascend.app.trainerpro.monthly` | MYR 99.99 |

Trainer Pro is the higher subscription level. StoreKit supplies localized names, descriptions and prices; frontend code never presents a hardcoded Apple price. Confirm available Apple price points before release. Both products require localization, availability, review screenshots and submission with the first app version offering purchases.

## Implementation

- Native StoreKit 2 plugin: product retrieval, app-account-linked purchase, unfinished transaction recovery, explicit restore and Apple's subscription management sheet.
...
Configure App Store Server Notifications **V2** production and sandbox URLs at `<backend>/api/v1/webhooks/apple` after deploying the intended backend. Do not point Apple at a nonexistent endpoint. TestFlight uses sandbox transactions, so test accounts must be explicitly allowed. Outside-app streamlined purchasing must be disabled until unlinked purchases have a supported account-linking flow.
...
5. After 1.0 is released, create App Store version 1.1 and submit the first subscriptions and group with that version. Update review notes to accurately describe IAP and provide the review login.
